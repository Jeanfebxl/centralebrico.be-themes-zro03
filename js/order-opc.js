		...
		success: function(json)
		{
			...
			else
			{
				...
				$('#opc_new_account').fadeOut('fast', function() {
					...
					// 1.5 template
					if (json.block_user_info !== '' && $('#header_user_info').length == 1)
					{
						$('#header_user_info').replaceWith(json.block_user_info);
					}

					// 1.6 temmplate
					if (json.block_user_info_nav !== '' && $('#nav_user_info').length == 1)
					{
						$('#nav_user_info').replaceWith(json.block_user_info_nav);
					}
